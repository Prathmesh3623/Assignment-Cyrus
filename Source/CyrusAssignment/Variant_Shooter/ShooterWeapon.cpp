...
void AShooterWeapon::StartFiring()
{
	// Fire line trace
	FHitResult Hit;
	FVector Start = GetOwner<APawn>()->GetActorLocation(); // Or better: use camera or muzzle
	FVector Forward = GetOwner<APawn>()->GetControlRotation().Vector();
	...
	{
		// Try damaging the box
		if (ABoxTarget* HitBox = Cast<ABoxTarget>(Hit.GetActor()))
		{
			HitBox->ApplyDamage(1);  // Deal 1 damage to the box
		}
	}
	if (!ProjectileClass || !PawnOwner) return;

	// Get location and rotation
	FVector MuzzleLocation = FirstPersonMesh->GetSocketLocation(MuzzleSocketName);
	FRotator MuzzleRotation = PawnOwner->GetControlRotation(); 
...
// Play muzzle flash
	if (NiagaraMuzzleFlash)
	{
		UNiagaraFunctionLibrary::SpawnSystemAttached(
			NiagaraMuzzleFlash,
			GetFirstPersonMesh(),       // or ThirdPersonMesh, depending on camera
			MuzzleSocketName,
			FVector::ZeroVector,
	...
	// Spawn bullet tracer from Start to Hit (or End if no hit)
	if (TracerEffect)
	{
		FVector TraceEnd = Hit.bBlockingHit ? Hit.ImpactPoint : End;

		// Spawn at start, rotate toward impact point
		FRotator ShotDirection = (TraceEnd - Start).Rotation();
